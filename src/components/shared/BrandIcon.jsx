...
export function BrandIcon({ id, size = 20 }) {
  ...
  if (!render) return WRAP('#8a8a9a', <span style={{ color: '#fff', fontSize: 11, fontWeight: 700 }}>{(id || '?')[0]?.toUpperCase()}</span>);
  return render(size);
}
