...
import { flightProgress, pacingRatio, projectedFinalSpend, pacingStatus } from './pacing.js';
...
describe('pacingRatio', () => {
  ...
  it('is null when the flight has not started', () => {
...
describe('pacingStatus', () => {
  it('is null for a null ratio', () => {
    expect(pacingStatus(null)).toBeNull();
  });

  it('is behind under 0.6', () => {
    expect(pacingStatus(0.3)).toBe('behind');
  });

  it('is on_pace between 0.6 and 1.4', () => {
    expect(pacingStatus(0.6)).toBe('on_pace');
    expect(pacingStatus(1)).toBe('on_pace');
    expect(pacingStatus(1.4)).toBe('on_pace');
  });

  it('is ahead over 1.4', () => {
    expect(pacingStatus(2)).toBe('ahead');
  });
});
