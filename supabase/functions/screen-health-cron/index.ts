...
// Thresholds
const OFFLINE_MINUTES = 60;   // screen not seen for 60+ min → offline
const IDLE_MINUTES    = 5;    // seen 5-60 min ago → idle
...
Deno.serve(async (_req: Request) => {
  ...
  // Fetch all screens that have ever been seen (have a last_seen value)
  ...
  for (const screen of screens) {
    ...
    if (lastSeen < offlineCutoff) {
      newStatus = "offline";
    } else if (lastSeen < idleCutoff) {
      newStatus = "idle";
    } else {
      newStatus = "online";
    }
  ...
  // Bulk update health_status
  for (const u of updates) {
    await supabase
      .from("screens")
      .update({ health_status: u.health_status })
      .eq("id", u.id);

    // Notify operator when screen transitions to offline
