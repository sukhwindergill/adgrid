# Pre-Hardware Cleanup Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Close remaining product gaps before hardware build — operator settings page, mobile layout polish on Revenue/Billing, and DB migration push to prod.

**Architecture:** Operator settings reuses the advertiser settings pattern (profile/security/notifications/team tabs) plus a Payouts tab for Stripe Connect status. Revenue and Billing get `useBreakpoint` wired in to make grids responsive. DB migration is a one-command push.

**Tech Stack:** React, Supabase, Stripe Connect, `useBreakpoint` hook (`src/lib/useBreakpoint.js`)
...
| File | Action |
|------|--------|
| `supabase/` | CLI push — no code change |
| `src/views/operator/OperatorSettingsView.jsx` | Create new |
| `src/components/layout/Sidebar.jsx` | Modify — add `op-settings` to operator nav |
| `src/App.jsx` | Modify — add `op-settings` route |
| `src/views/operator/Revenue.jsx` | Modify — add mobile responsive grids |
| `src/views/operator/Billing.jsx` | Modify — add mobile responsive grids |

---

### Task 1: Push DB migration to production
...
Expected output: migration `20260520000000_add_notification_prefs` applied successfully. If it says already applied, that's fine — move on.
...
No files changed — migration was already in repo. Nothing to commit.

---

### Task 2: OperatorSettingsView — new file

**Files:**
- Create: `src/views/operator/OperatorSettingsView.jsx`
...
git commit -m "feat: add OperatorSettingsView with profile/security/notifications/team/payouts tabs"
```

---

### Task 3: Wire op-settings into Sidebar and App.jsx

**Files:**
- Modify: `src/components/layout/Sidebar.jsx`
- Modify: `src/App.jsx`

- [ ] **Step 1: Add op-settings to operator nav in Sidebar.jsx**

In `src/components/layout/Sidebar.jsx`, find the operator nav array (around line 30–45). Add `op-settings` entry after `display`:
...
- [ ] **Step 2: Add op-settings route in App.jsx**

In `src/App.jsx`, add the import near the other operator view imports:

```js
import { OperatorSettingsView } from './views/operator/OperatorSettingsView.jsx';
```

Then in the view routing function (operator section, around line 362), add before the final `return <Dashboard...`:
...
git commit -m "feat: wire op-settings route and sidebar nav for OperatorSettingsView"
```

---

### Task 4: Mobile polish — Revenue.jsx
...
Add import at top of `src/views/operator/Revenue.jsx`:

```js
import { useBreakpoint } from '../../lib/useBreakpoint.js';
```

- [ ] **Step 2: Destructure isMobile in Revenue component**

Inside `export function Revenue(...)`, add after the `useState` line:

```js
const { isMobile } = useBreakpoint();
```

- [ ] **Step 3: Make KPI grid responsive**
...
- [ ] **Step 4: Make Revenue Split / By City grid responsive**
...
```bash
git add src/views/operator/Revenue.jsx
git commit -m "fix: make Revenue view responsive on mobile"
```

---

### Task 5: Mobile polish — Billing.jsx
...
Add import at top of `src/views/operator/Billing.jsx`:

```js
import { useBreakpoint } from '../../lib/useBreakpoint.js';
```

- [ ] **Step 2: Destructure isMobile in Billing component**

Inside `export function Billing(...)`, add after the first `useState` / `useBilling` call:

```js
const { isMobile } = useBreakpoint();
```

- [ ] **Step 3: Make KPI row responsive**
...
- [ ] **Step 4: Make charts/summary grid responsive**
...
git commit -m "fix: make Billing view responsive on mobile"
```
...
- [ ] `notification_prefs` column confirmed in production DB
- [ ] Operator sidebar shows Settings nav item
- [ ] Settings page loads with 5 tabs: Profile / Security / Notifications / Team / Payouts
- [ ] Payouts tab shows Stripe Connect status and button
- [ ] Revenue page renders in 2-column grid on mobile (not 4-column overflow)
- [ ] Billing page renders in 2-column grid on mobile (not 4-column overflow)
